...
public class CopyOfMethodLocker {
	...
	//	String argsOfToy2 = "-f J -pp -cp /home/lpxz/eclipse/workspace/Playground/bin:/home/lpxz/eclipse/workspace/soot24/bin Toy2";// soot.jimple.toolkits.thread.synchronizationLP.Jimples.HelloWorld";
		String argsOfToy2 = "-app -f J -pp -cp .:/home/lpxz/eclipse/workspace/simple/bin example.Example";// soot.jimple.toolkits.thread.synchronizationLP.Jimples.HelloWorld";
				...
				for(SootClass appClass : Scene.v().getApplicationClasses())
				{
		    	    for(SootMethod sm : appClass.getMethods())
		    	    {
						if(sm.isConcrete() && sm.hasActiveBody())
						{
							...
							if(sm.getName().equals("parse")) //sm.getDeclaringClass().getName().equals("example.URLParse")
...
//		        System.out.println(b.toString());
			...
			private void change2locallock(CriticalSection cSection)
			{
				...
				Local lockObj = Jimple.v().newLocal("localizedLock_" + (serialNo++),
			...
			private void change2Locallock(SootMethod method) {				
				Body b = method.retrieveActiveBody();
				UnitGraph  eug = new EnhancedUnitGraph(
						b);
...
//		}
	...
	// ========================================
	private static void monitorenter(SootMethod sm, boolean random) {
		...
		if(sm.isStatic())
		{
			...
			else
			{
				 //Thread.currentThread();
				  SootMethod curentThreadMethod = Scene.v().getMethod("<" + "java.lang.Thread" + ": java.lang.Thread " + "currentThread" + "()>");
				 right = Jimple.v().newStaticInvokeExpr(curentThreadMethod.makeRef());
				  left = Jimple.v().newLocal(LOCALLOCK+sm.getName(), RefType.v("java.lang.Thread"));//!!problematic!!
					if(!body.getLocals().contains(left))
					{
						body.getLocals().add(left);
					}
...
//			}
			...
			else
			{
				  ...
				  SootMethod curentThreadMethod = Scene.v().getMethod("<" + "java.lang.Thread" + ": java.lang.Thread " + "currentThread" + "()>");
					 right = Jimple.v().newStaticInvokeExpr(curentThreadMethod.makeRef());
                left = Jimple.v().newLocal(LOCALTHISLOCK+sm.getName(), RefType.v("java.lang.Thread"));//!!problematic!!
				if(!body.getLocals().contains(left))
				{
					body.getLocals().add(left);
				}
